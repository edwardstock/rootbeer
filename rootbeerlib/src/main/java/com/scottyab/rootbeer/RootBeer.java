...
public class RootBeer {
    ...
    public boolean hasProp(final String propName) {
        for (String prop : getProps()) {
            if (prop.equalsIgnoreCase(propName)) {
                return true;
            }
        }
    ...
    /**
     * Get android system property (getprop)
     *
     * @param propName prop name. for example: service.adb.tcp.port
     * @return null if property has not found, exception if can't get property, otherwise string value
     * @throws IOException          Throws if can't run executable
     * @throws InterruptedException Throws if can't read buffer from executed command
     */
    public String getProp(final String propName) throws IOException, InterruptedException {
        if (!hasProp(propName)) return null;
        return runBin("getprop", propName);
    }

    /**
     * Set android system property (setprop)
     *
     * @param propName prop name. for example: service.adb.tcp.port
     * @param value    prop value
     * @return false if prop not found or exception throws, true if success written new value
     */
    public boolean setProp(final String propName, final String value) {
        if (!hasProp(propName)) return false;

        try {
            runBin("setprop", propName, value);
            return true;
        } catch (IOException e) {
            QLog.e("Can;t execute setprop", e);
        } catch (InterruptedException e) {
            QLog.e("Can't read command result", e);
        }
    ...
    /**
     * Executes system binary
     *
     * @param cmd  command name
     * @param args arguments with values in free-form. Example: --myarg=val, -p val, -h
     * @return string command result
     * @throws IOException          Throws if can't run executable
     * @throws InterruptedException Throws if can't read buffer from executed command
     */
    public String runBin(final String cmd, String... args) throws IOException, InterruptedException {
        final StringBuilder argBuilder = new StringBuilder();
        for (String arg : args) argBuilder.append(" ").append(arg);

        Runtime runtime = Runtime.getRuntime();
        Process process = runtime.exec(String.format("%s%s", cmd, argBuilder.toString()));
        process.waitFor();
    ...
    /**
     * Fetches and transforms props values to map
     * @return Hash map: key=propname, value=propvalue
     */
    public HashMap<String, String> fetchProps() {
        final Pattern p = Pattern.compile("^\\[(.*)\\].+\\[(.*)\\]");
        final String[] props = getProps();
        if(mProps == null) {
            mProps = new HashMap<>(props.length);
        } else {
            mProps.clear();
        }
    ...
    public String[] getProps() {
        ...
        if(propval == null) return new String[0];
